...
const Category = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('/categories', data, { headers: { 'Content-Type': 'multipart/form-data' } });

      if (res.data.success) {
        Swal.fire('Success!', 'Category added successfully', 'success');
  
        // Reset form
        setFormData({ image_url: null, name: '', status: '' });
  ...
  const handleChangeStatus = (id, newStatus, message) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.put(`/categories/${id}/status`, { newStatus });
          if (res.data.success) {
  ...
  //     try {
  //       const res = await axios.delete('/categories/bulkcategories', { data: { ids: selectedIds } });
  //       Swal.fire('Deleted!', res.data.message, 'success');
  //       fetchData(); // reload table
  //       setSelectedIds([]); // clear selection
  //     } catch (err) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`/categories/${formData.data_id}`, data, {
    ...
    <>
      ...
      <div className="modal fade bd-example-modal-lg" tabIndex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div className="modal-dialog modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="myLargeModalLabel">Add Category</h5>
              <button type="button" className="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
